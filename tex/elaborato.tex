...
Throughout history, questions regarding the beginning and evolution of our universe have captivated humanity. Early answers took the form of creation myths which, though often imaginative, lacked a concrete empirical basis.
It was not until the 20th century that cosmology emerged as a rigorous discipline, driven by advances in astronomy and Einstein's formulation of a new theory of gravity.
\para
...
\para
To formally define isotropy, we then begin by introducing a timelike curve (i.e., an observer) with tangent vector $\bar{u}$. We will call it an \emph{isotropic observer} if, for all points $p$ in its trajectory, and any two unit “spatial" (that is, belonging to the orthogonal space of $\bar{u}$) vectors $\bar{s}_1$, $\bar{s}_2 \in V_p$ an isometry exists that sends $\bar{s}_1$ to $\bar{s}_2$ and leaves $p$ and $\bar{u}$ fixed. An immediate consequence of this definition is that there cannot exist a preferred vector in the orthogonal space of $\bar{u}$.
\para
If a homogeneous universe admits one isotropic observer, say intersecting the hypersurface $\Sigma_t$ at a point $p$, it must, by homogeneity, admit an isotropic observer passing through every other point of the surface. Therefore, there must exist a congruence of timelike isotropic curves filling spacetime. 
\para
Furthermore, if we assume that the homogeneous foliation $\Sigma_t$ is unique, the timelike curves must be everywhere orthogonal to the surfaces of homogeneity; if this were not the case, the normal vector of $\Sigma_t$ would pick a special direction in the orthogonal space of $\bar{u}$, violating isotropy. As a consequence, the tangent space of $\Sigma_t$ coincides with the orthogonal space of $\bar{u}$. Isotropy then tells us that it's impossible to have a preferred vector in the tangent space of $\Sigma_t$.
\para
...
\]
It is straightforward to show that $L$ is a symmetric linear map with respect to this inner product, i.e. $\braket{L(\tilde{w}), \tilde{\eta}} = \braket{\tilde{w}, L(\tilde{\eta})}$. This follows from the property of the Riemann tensor to be symmetric under the swap of the first two and last two indices, $\tensor{R}{_{ij}_{ml}} = \tensor{R}{_{ml}_{ij}}$. We recall that a symmetric linear map, according to the spectral theorem, is always diagonalizable. This means that there must be some 2-forms that are eigenvectors of $L$. 
If we had two eigenvectors with different
eigenvalues we could distinguish them, 
and then we would be able to distinguish two vectors in the tangent space, since, in three dimensions, there is a natural map between 2-forms and vectors obtained through the Hodge dual, $w_{ij} \to v_k= \frac{1}{2} \sqrt{\det(h)} \epsilon_{kij}w^{ij}$, where $\epsilon_{kij}$ is the totally antisymmetric symbol.
This would violate isotropy, so we conclude that all the eigenvalues are the same, or equivalently, that $L = 2K I$, where $2K$ is the common eigenvalue and $I$ is the identity. This fixes the form of the Riemann tensor as
\[
...
\end{equation}
By contracting the indices, we find that $K = \frac{R}{6}$, where $R$ is the scalar curvature. Since $\Sigma_t$ is homogeneous, $K$ must be constant over the whole surface. A manifold satisfying Eq. \eqref{const_curvature} with $K$ constant is called a space of constant curvature. It can be shown that 
any two spaces of constant curvature with the same dimension, metric signature, and value of $K$ are locally isometric (see Eisenhart, 1949, as cited in \cite{wald-relativity}). Thus, we only need to identify one representative metric for each value of $K$. We can classify these metrics in three categories: $K >0$, $K = 0$ and $K<0$.
\begin{itemize}
    ...
    \item $K = 0$ \textbf{(flat universe)}: As one may expect, three-dimensional Euclidean space has constant zero curvature. Taking spherical coordinates to highlight the similarity with the other metrics, we have 
    \[
...
Having classified all possible metrics of the homogeneous surfaces, we now seek to determine the metric of the full spacetime. Any vector $\bar{v}$ can be decomposed into a component $\bar{v}_\parallel$ parallel to the isotropic observer's tangent  vector $\bar{u}$, and an orthogonal component $\bar{v}_\perp$. We then find that 
\begin{equation}
...
\para
In these coordinates, using Eq. \eqref{metric_decomposition}, the metric becomes 
\begin{equation}
...
\end{cases}
\]
Eq. \eqref{FLRW1} defines the Friedmann-Lemaître-Robertson-Walker (FLRW) metric.
...
\para
It is convenient to re-express the FLRW metric by introducing a new coordinate $r := S_k(\chi)$. Eq. \eqref{FLRW1} then becomes
\begin{equation}
...
Despite its name, the coordinate $r$ is dimensionless, while $a(t) \sim [L]$. Notice that the areal radius in these coordinates is \[
    r_A = a(t) r
\]
while the infinitesimal \emph{instantaneous physical distance}, the spatial distance of two radially separated points at fixed $t$, is \[
...
\para
Through the usual procedure, it's possible to calculate the Christoffel symbols, the Riemann and Ricci tensors, and the scalar curvature associated to the metric \eqref{FLRW2}. This is done in Appendix \ref{appA}
...
\begin{equation}
    ...
    \label{EM_tensor}
\end{equation}
where $p$ is the pressure and $\rho$ the density of the fluid at rest, and $u^\mu$ its four-velocity. To be consistent with our assumption of isotropy and homogeneity, $\rho$ and $p$ must be constant in space, and the fluid must be at rest with respect to the isotropic observers, so $u^\mu = (1,0,0,0)$ in our chosen coordinates. Equation \eqref{EM_tensor} then becomes
\begin{equation}
T_{\mu\nu} = \begin{pmatrix} 
\rho & \; 0 \; & \; 0 \; & \; 0 \; \\ 
0 & & & \\ 
...
\end{equation}
where the last equality is derived from the results of Appendix \ref{appA}.
\para
...
\para
Integrating both sides of Eq. \eqref{rho_hubble_relation} we obtain
\begin{equation}
    \rho \propto a^{-3(1+w)}
    \label{rho_a_prop}
\end{equation}
...
\para
The second fluid we consider is radiation, which is made up of photons. Since photons are just excitations of the electromagnetic field, the energy-momentum tensor must coincide with (see section 1.10 of \cite{carroll-spacetime}):
\[
    T^{\mu \nu} = F^{\mu \lambda} F^{\nu}{}_{\lambda} - \frac{1}{4} g^{\mu \nu} F^{\lambda \sigma}F_{\lambda \sigma}
\]
which is traceless. Therefore, by Eq. \eqref{EM_trace} we find $w = \frac{1}{3}$, and from \eqref{rho_a_prop} we obtain \[
    \rho_r \propto a^{-4}
\]
...
\para
It is also possible to treat dark energy as a perfect fluid, with density \[\rho_{\Lambda} = -p_{\Lambda} =  \frac{\Lambda}{8 \pi G_N}\] where $\Lambda$ is the cosmological constant and $G_N$ Newton's constant. Since $w = -1$, Eq. \eqref{rho_a_prop} implies
\[
...
We finally turn to the Einstein field equations \[
    R_{\mu \nu}  - \frac{1}{2} R g_{\mu \nu} = 8 \pi G_N T_{\mu \nu}
\]
By taking the trace we obtain $R = - 8 \pi G_N T$, where $T = T^\mu{}_\mu$. Substituting this expression back into the Einstein equations we obtain \begin{equation}  
R_{\mu \nu} = 8 \pi G_N (T_{\mu\nu} - \frac{1}{2}T g_{\mu\nu})
\label{EE}
\end{equation}

We again make use of the results in Appendix \ref{appA}
to compute the $\mu\nu = ij$ and $\mu\nu = 00$ components of Eq. \eqref{EE}, from which we can derive
\begin{equation}
...
\begin{equation}
    ...
    \label{F2}
\end{equation}
These are known as the Friedmann equations. These equations are not independent: if we have a solution $a(t)$ of Eq. \eqref{F1}, then we automatically have a solution to Eq. \eqref{F2}. To see this, we differentiate \eqref{F1}, finding 
\[
6 \frac{\dot{a}}{a} \left( \frac{\ddot{a}}{a} - \frac{\dot{a}^2}{a^2} - \frac{k}{a^2} \right) = 8\pi G_N \dot{\rho}
\]
Now we substitute the second and third terms by using Eq. \eqref{F1}, obtaining
\[
    6 \frac{\dot{a}}{a} \left( \frac{\ddot{a}}{a} - \frac{8\pi G_N}{3} \rho \right) = 8\pi G_N \dot{\rho}
\]
Then we use Eq. \eqref{E_conservation} to replace $\dot{\rho}$ on the right side. After some algebraic manipulations, we recover \eqref{F2}.
\para
We can make the Friedmann equations look cleaner by introducing the \emph{critical density} $\rho_\text{crit} = \frac{3 H^2}{8\pi G_N}$, and the \emph{density parameter} $\Omega = \frac{\rho}{\rho_\text{crit}}$. With these definitions Eq. \eqref{F1} becomes
\begin{equation}
     \Omega - 1 =  \frac{k}{a^2H^2}
     \label{FOmega}
\end{equation}
This expression implies that the ratio between $\rho$ and $\rho_c$ determines the geometry of the universe in the following way:
\begin{itemize}
...
provide the bound $|\Omega-1| < 0.01$ \cite{planck2018}.
\para
The fluids we have discussed in the previous section coexist, each adding its contribution to the $\rho$ term in the Friedmann equation. But, since they evolve at different rates with respect to the scale factor $a$, there are some periods in which some fluids dominate, and all the others are negligible. This motivates us to consider solutions of the Friedmann equation with only one fluid in a flat universe ($k=0$).
\para
Under these assumptions it is straightforward to solve the Eq. \eqref{F1}. We use the relation \eqref{rho_a_prop} to substitute $\rho$ on the right side, obtaining a differential equation that can be integrated, finding: \begin{equation}
    \begin{cases} 
...
\section{FLRW Connection and Curvature}
\label{appA}
The Christoffel symbols, Riemann tensor, Ricci tensor, and Ricci scalar were computed using \emph{Mathematica}. The corresponding notebook is available at \url{https://github.com/LazyLagrangian/cosmology_brief_intro}.
\para
We summarize here the main results. Components not explicitly listed are either zero or determined by symmetry. The non-vanishing Christoffel symbols are

\begin{equation}
\begin{aligned}
\Gamma^0_{11} &= \frac{a\dot{a}}{1-k r^2} & \Gamma^1_{11} &= \frac{k r}{1-k r^2} \\
\Gamma^0_{22} &= a\dot{a}r^2 & \Gamma^0_{33} &= a\dot{a}r^2 \sin^2 \theta \\
\Gamma^1_{01} &= \Gamma^2_{02} = \Gamma^3_{03} = \frac{\dot{a}}{a} \\
\Gamma^1_{22} &= -r(1-k r^2) & \Gamma^1_{33} &= -r(1-k r^2) \sin^2 \theta \\
\Gamma^2_{12} &= \Gamma^3_{13} = \frac{1}{r} & \\
\Gamma^2_{33} &= -\sin \theta \cos \theta & \Gamma^3_{23} &= \cot \theta,
\end{aligned}
\end{equation}
the Ricci tensor
\begin{equation}
...
R_{11} &= \frac{a\ddot{a} + 2\dot{a}^2 + 2k}{1 - k r^2} \\
...
\bibitem{casadio-notes}
R.~Casadio, \textit{General Relativity 1}, Lecture notes, University of Bologna, 2025.

\bibitem{wald-relativity}
R.~M. Wald, \textit{General Relativity}, University of Chicago Press, 1984.

\bibitem{carroll-spacetime}
